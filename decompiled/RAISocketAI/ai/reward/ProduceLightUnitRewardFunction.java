...
public class ProduceLightUnitRewardFunction
extends RewardFunctionInterface {
    public void computeReward(int n, int n2, TraceEntry traceEntry, GameState gameState) {
        ...
        for (Pair pair : traceEntry.getActions()) {
            if (((Unit)pair.m_a).getPlayer() != n || ((UnitAction)pair.m_b).getType() != 4 || ((UnitAction)pair.m_b).getUnitType() == null || !((UnitAction)pair.m_b).getUnitType().name.equals("Light")) continue;
            this.reward += 1.0;
        }
    }
