...
extends RAITournament {
    public RAIRoundRobinTournament(List<AI> list) {
        super(list);
    }
    ...
    public static void main(String[] stringArray) throws Exception {
        ...
        int n2 = rAITournamentArguments.getOptionInteger('t', 100);
        boolean bl = !rAITournamentArguments.hasOption('T');
        UnitTypeTable unitTypeTable = new UnitTypeTable(2);
        AI[] aIArray = new AI[]{new RAISocketAI(n2, -1, unitTypeTable, rAITournamentArguments.getOptionInteger('p', 0), rAITournamentArguments.pythonVerbosity, rAITournamentArguments.hasOption('b')), new POWorkerRush(unitTypeTable), new POLightRush(unitTypeTable), new CoacAI(unitTypeTable), new mayari(unitTypeTable)};
        RAIRoundRobinTournament rAIRoundRobinTournament = new RAIRoundRobinTournament(Arrays.asList(aIArray));
        ArrayList<Pair> arrayList = new ArrayList<Pair>();
        arrayList.add(new Pair((Object)"maps/8x8/basesWorkers8x8A.xml", (Object)3000));
        arrayList.add(new Pair((Object)"maps/8x8/FourBasesWorkers8x8.xml", (Object)3000));
        arrayList.add(new Pair((Object)"maps/NoWhereToRun9x8.xml", (Object)4000));
        arrayList.add(new Pair((Object)"maps/16x16/basesWorkers16x16A.xml", (Object)4000));
        arrayList.add(new Pair((Object)"maps/16x16/TwoBasesBarracks16x16.xml", (Object)4000));
        arrayList.add(new Pair((Object)"maps/DoubleGame24x24.xml", (Object)5000));
        arrayList.add(new Pair((Object)"maps/BWDistantResources32x32.xml", (Object)6000));
        arrayList.add(new Pair((Object)"maps/BroodWar/(4)BloodBath.scmB.xml", (Object)8000));
        ...
        int n3 = 0;
        while (((File)(object = new File((String)object2 + ++n3))).exists()) {
        }
        ((File)object).mkdir();
        ...
        int n4 = rAITournamentArguments.getOptionInteger('n', 10);
        FileWriter fileWriter = new FileWriter(file);
        PrintWriter printWriter = new PrintWriter(System.out);
        if (printWriter != null) {
            ((Writer)printWriter).write(rAIRoundRobinTournament.getClass().getName() + ": Starting tournament\n");
        }
        fileWriter.write(rAIRoundRobinTournament.getClass().getName() + "\n");
        ...
        fileWriter.write("iterations\t" + n4 + "\n");
        fileWriter.write("timeBudget\t" + n2 + "\n");
        ...
        fileWriter.write("timeoutCheck\t" + bl + "\n");
            ...
            for (int i = 0; i < arrayList.size(); ++i) {
                ...
                for (int j = 0; j < aIArray.length; ++j) {
                    for (int k = 0; k < aIArray.length; ++k) {
                        ...
                        rAIRoundRobinTournament.playSingleGame((Integer)pair3.m_b, n2, -1, 10000L, 1000L, true, bl, true, true, unitTypeTable, string2, fileWriter, printWriter, stringArray2, blArray, n, i, physicalGameState, j, k);
                    }
                }
            }
        }
        rAIRoundRobinTournament.printEndSummary(arrayList.stream().map(pair -> (String)pair.m_a).collect(Collectors.toList()), n4, fileWriter, printWriter);
    }
}
