    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            values[step] = agent.get_value(
                obs[step], ScFeatures[step], zFeatures[step]).flatten()
        ...
        for info in infos:
            ...
            if 'episode' in info.keys():
                ...
                # ca. 0.9 bei 2000 und 1.1 bei 500
                winloss = winloss * (-0.00013 * game_length + 1.16)
                print(
                    f"global_step={global_step}, episode_reward={info['microrts_stats']['RAIWinLossRewardFunction'] * winloss + info['microrts_stats']['AttackRewardFunction'] * attack}")
                writer.add_scalar(
                    "charts/old_episode_reward",
                    info['episode']['r'],
                    global_step)
                writer.add_scalar(
                    "charts/Game_length",
                ...
                writer.add_scalar(
                    ...
                    info['microrts_stats']['RAIWinLossRewardFunction'] *
                    winloss +
                    info['microrts_stats']['AttackRewardFunction'] *
                    attack,
                    global_step)
                writer.add_scalar(
                    "charts/AttackReward",
