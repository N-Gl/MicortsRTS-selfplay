...
def logging_PPO_update(args, experiment_name, writer, CHECKPOINT_FREQUENCY, agent, optimizer, loss, global_step, start_time, 
                       update, i_epoch_pi, approx_kl, pg_loss, entropy_loss, v_loss, kl_loss):
...
def logging_PPO_training(writer, global_step, infos, winloss, attack):
    for info in infos:
...
def gae(args, device, dones, rewards_attack, rewards_winloss, values, next_done, last_value):
...
def apply_invalid_action_masks(step, invalid_action_masks, real_action):
...
if BCtraining:
    ...
    if newdata:
        opponents =[
                    # neu
                    # [microrts_ai.workerRushAI,microrts_ai.workerRushAI], [microrts_ai.workerRushAI,microrts_ai.passiveAI],[microrts_ai.workerRushAI,microrts_ai.lightRushAI], [microrts_ai.workerRushAI,microrts_ai.coacAI], [microrts_ai.workerRushAI,microrts_ai.mayari], [microrts_ai.workerRushAI,microrts_ai.randomAI],[microrts_ai.workerRushAI,microrts_ai.randomBiasedAI],[microrts_ai.workerRushAI, microrts_ai.rojo],
                    ...
                    # [microrts_ai.coacAI, microrts_ai.mayari], [microrts_ai.mayari, microrts_ai.coacAI]
                    ]
        ...
        # für jede AI Kombination in opponents ein Environment erstellen und Spiele durchführen
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        java_valid_actions = apply_invalid_action_masks(step, invalid_action_masks, real_action)
        ...
        logging_PPO_training(writer, global_step, infos, winloss, attack)
    ...
    with torch.no_grad():
        ...
        advantages, returns = gae(args, device, dones, rewards_attack, rewards_winloss, values, next_done, last_value)
    ...
    if args.prod_mode:
        logging_PPO_update(args, experiment_name, writer, CHECKPOINT_FREQUENCY, agent, optimizer, loss, global_step, start_time,
                           update, i_epoch_pi, approx_kl, pg_loss, entropy_loss, v_loss, kl_loss)
