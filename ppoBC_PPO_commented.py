...
class Agent(nn.Module):
    ...
    def get_action(
        ...
        invalid_action_masks = invalid_action_masks.view(
            -1, 256, envsT.action_plane_space.nvec.sum() + 1)
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            values[step] = agent.get_value(
                obs[step],
                ScFeatures[step],
                zFeatures[step]).flatten()  # critic(forward(...))
            ...
            action, logproba, _, invalid_action_masks[step] = agent.get_action(
                obs[step], ScFeatures[step], zFeatures[step], envs=envsT)
...
# =========================
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            # --clip_coef
            # it is the same, because in loss1 and 2 there is a minus sign and
            # max is used at the end
