...
if __name__ == "__main__":
    ...
    parser.add_argument('--total-timesteps', type=int, default=31000000, # new (BA Parameter) (Trainingwas run for 30 million steps) (hier: + 1 million buffer steps)
    ...
    parser.add_argument('--num-bot-envs', type=int, default=24, # new (BA Parameter) (number of parallel game environments = 24) (TODO: 16 CoacAI and 8 Mayari environments)
...
args.minibatch_size = int(args.batch_size // args.n_minibatch) # new (BA Parameter) (minibatch size = 3072 (=(num_envs*num_steps)/ n_minibatch = (24*512)/4))
...
envs = MicroRTSGridModeVecEnv(
    ...
    render_theme=1,
    ai2s=[microrts_ai.coacAI for _ in range(16)] + [microrts_ai.mayari for _ in range(8)], # new (BA Parameter) (Targeted training during PPO training) 16 CoacAI and 8 Mayari environments
...
if BCtraining:
    if newdata:
        ...
        # TODO: for BC finetuning (save the expertdata of both segments and Train the BC agent with newdata == False afterwards) (nehme die heraus, die gegen die nicht gewonnen wird?)
        # new (BA Parameter) 4100 replays with nurwins == True (num_runs, opponents ist gleich)

        
        # temp
        # num_runs = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]
    ...
    # for epoch in range(start_epoch, start_epoch + 1000): # 1000 Epochen
    # temp
    for epoch in range(start_epoch, start_epoch + 100): # new (BA Parameter) 100 Epochen, TODO: BC-finetuning 70 Epochen (sicher: 80)
        train_loss_sum = 0.0
        train_count = 0
        ...
        # new (_epoch_{epoch})
        os.makedirs(f"models/{experiment_name}_epoch_{epoch}", exist_ok=True)
        torch.save(agent.state_dict(), f"models/{experiment_name}_epoch_{epoch}/agent.pt")
...
num_updates = args.total_timesteps // args.batch_size # 10000000000 // 12288 = 813802
...
for update in range(starting_update, num_updates + 1):
    ...
    if args.prod_mode and update % CHECKPOINT_FREQUENCY == 0:
        print("checkpoint")
        # new (_update_{update})
        os.makedirs(f"models/{experiment_name}_update_{update}", exist_ok=True)
        torch.save(agent.state_dict(), f"models/{experiment_name}_update_{update}/agent.pt")
